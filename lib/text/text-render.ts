...
export class TextRenderer {
  ...
  glyphWidth = (glyph: GlyphMatrix) => (!glyph.length ? SPACE_WIDTH : Math.max(...glyph.map(([x]) => x)) + 1)
  ...
  string(
    ...
    const width = glyphs.reduce((a, b) => a + this.glyphWidth(b), 0) + (str.length - 1) * this.KERNING
  ...
  box(
    ...
    const width = Math.max(
      ...strings.map((str) => str.reduce((w, g) => w + this.glyphWidth(g), 0) + (str.length - 1) * this.KERNING)
    )
    const height =
      strings.reduce((h, s) => h + Math.max(...s.map(this.glyphHeight)), 0) + (strings.length - 1) * this.LINE_SPACING
